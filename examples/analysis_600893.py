...
if fin is not None and len(fin) > 0:
    ...
    print(f"  每股净资产: {r.get('meigujingzichan','N/A')}")
    print(f"  净利润: {r.get('jinglirun','N/A')}")
    print(f"  主营收入: {r.get('zhuyingshouru','N/A')}")
    print(f"  ROE: {r.get('roe','N/A')}")
    print(f"  总股本: {r.get('zongguben','N/A')}")
    print(f"  流通股: {r.get('liutongguben','N/A')}")
    print(f"  每股公积金: {r.get('meigugongjijin','N/A')}")
    print(f"  每股未分配利润: {r.get('meiguweifeipeili','N/A')}")
    print(f"  股东人数: {r.get('gudongrenshu','N/A')}")
...
if above:
    ...
    print(f"\n上方抛压位: {mp}元 (占比{above[mp]/tc*100:.1f}%) 上方合计{at:.1f}%")
if below:
...
try:
    ...
    if not df_eps.empty:
        ...
        # 估值计算
        price_val = float(vals[3])
        pe_ttm = float(vals[39]) if vals[39] else 0
        years_sorted = sorted(df_eps["年度"].unique())
        ...
        if eps_cur and eps_cur > 0:
            ...
            print(f"\n估值指标:")
            print(f"  前向PE(基于{years_sorted[0]}一致预期): {pe_fwd:.1f}x")
            print(f"  EPS增速(CAGR): {cagr*100:.1f}%")
            print(f"  PEG: {peg:.2f}" if peg != float('inf') else "  PEG: N/A")
            print(f"  PE消化到30x: {digest:.1f}年" if digest > 0 else "  PE消化到30x: 已低于30x")
    else:
        print("无机构覆盖数据")
...
try:
    ...
    if not df_fin.empty:
        df_annual = df_fin[df_fin['报告期'].str.contains('12-31')].head(5)
        for _, row in df_annual.iterrows():
            print(f"  {row['报告期']}: 营收={row.get('营业总收入','N/A')} (+{row.get('营业总收入同比增长率','N/A')}) | "
                  f"净利={row.get('净利润','N/A')} (+{row.get('净利润同比增长率','N/A')}) | "
                  f"EPS={row.get('基本每股收益','N/A')} ROE={row.get('净资产收益率','N/A')} 毛利率={row.get('销售毛利率','N/A')} 净利率={row.get('销售净利率','N/A')}")
except Exception as e:
    print(f"获取失败: {e}")
...
try:
    ...
    for rec in rows[:10]:
        eps_cur2 = rec.get('predictThisYearEps', 'N/A')
        eps_next2 = rec.get('predictNextYearEps', 'N/A')
        rating = rec.get('emRatingName', 'N/A')
        print(f"  {rec.get('publishDate','')[:10]} | {rec.get('orgSName','')} | {rec.get('title','')[:50]} | 评级:{rating} | EPS(今/明):{eps_cur2}/{eps_next2}")
except Exception as e:
    print(f"获取失败: {e}")
...
try:
    ...
    if str(d2.get("ResultCode", -1)) == "0":
        for block in d2.get("Result", []):
            ...
            names = [it.get("name", "") for it in items[:20]]
            print(f"  {btype}: {', '.join(names)}")
    else:
        print(f"获取失败: ResultCode={d2.get('ResultCode','')}")
except Exception as e:
    print(f"获取失败: {e}")
